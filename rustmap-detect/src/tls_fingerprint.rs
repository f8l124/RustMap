...
pub const TLS_PORTS: &[u16] = &[
    443, 8443, 993, 995, 465, 636, 989, 990, 992, 994, 5061, 8883,
];
...
mod tests {
    ...
    fn build_client_hello_contains_extensions() {
        ...
        let suites_len =
            u16::from_be_bytes([hello[suites_start], hello[suites_start + 1]]) as usize;
    ...
    fn parse_server_hello_with_session_id() {
        ...
        let record = build_test_server_hello([0x03, 0x03], &session_id, [0x00, 0x9C], 0, &exts);
    ...
    fn parse_server_hello_no_extensions() {
        let record = build_test_server_hello([0x03, 0x03], &[], [0x00, 0x2F], 0, &[]);
    ...
    fn client_hello_no_sni_without_hostname() {
        ...
        assert!(
            !found,
            "ClientHello without hostname should not contain SNI"
        );
    }
    ...
    fn parse_server_hello_alpn() {
        ...
        let record = build_test_server_hello([0x03, 0x03], &[], [0xC0, 0x2F], 0, &exts);
    ...
    fn compute_ja4s_tls13() {
        ...
        assert!(
            ja4s.starts_with("t1302_1301_"),
            "JA4S should start with t1302_1301_, got: {ja4s}"
        );
    ...
    fn compute_ja4s_tls12() {
        ...
        let ja4s = compute_ja4s(&fp);
        assert!(
            ja4s.starts_with("t1202_c02f_"),
            "JA4S should start with t1202_c02f_, got: {ja4s}"
        );
    }
    ...
    fn compute_ja4s_no_extensions() {
        ...
        let ja4s = compute_ja4s(&fp);
        assert!(
            ja4s.ends_with("_000000000000"),
            "No extensions should use fallback hash, got: {ja4s}"
        );
    }
    ...
    const TEST_CERT_DER: &[u8] = &[
        ,
        ...
        ,
    ...
    fn has_server_hello_done_detection() {
        ...
        let record2 = vec![
            0x16, 0x03, 0x03, 0x00, 0x04, 0x02, 0x00, 0x00,
            0x00, // ServerHello (type=0x02, len=0)
        ];
